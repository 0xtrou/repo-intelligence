 ...
 * and per-wisdom comparison. Derived values are computed, never stored.
 *
 * Usage: npm run insight -- <target> [--wisdom <w>] [--json]
 */
...
function main(): void {
  ...
  if (target === undefined) fail('missing target', 'usage: npm run insight -- <target> [--wisdom <w>] [--json]');
  ...
  console.log(`  plan: ${plan === null ? 'none' : plan.status}`);
  console.log('');
  ...
  if (wisdoms.length >= 2) {
    ...
    const singleFixture = fixturesSets.size <= 1;
    if (!singleFixture) {
